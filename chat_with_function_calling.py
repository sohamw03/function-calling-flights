...
def query(fromEntityId, toEntityId=None, departDate=None, wholeMonthDepart=None, market=None, locale=None, currency=None):
    ...
    d = {'fromEntityId': fromEntityId, 'toEntityId': toEntityId, 'departDate': departDate, 'wholeMonthDepart': wholeMonthDepart, 'market': market, 'locale': locale, 'currency': currency}
    for i in d:
        if d[i] is not None:
            req += i + '=' + str(d[i]) + '&'
    req = req[:-1]
...
def chat():
    ...
    openai.api_key = f"{os.getenv('OPENAI_API_KEY')}"  # Replace with your OpenAI API key
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": """Extract travel query parameters from the following input in this json format 
    ...
    # result = json.loads(result)["data"]["itineraries"]
